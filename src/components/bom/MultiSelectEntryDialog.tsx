...
export function MultiSelectEntryDialog({ open, onOpenChange, parentAssembly, onConfirm }: Props) {
  ...
  const toggleSelect = useCallback((id: string) => {
    setSelected(prev => {
      ...
      const item = activeTab === 'part'
        ? state.parts.find(p => p.id === id)
        : state.assemblies.find(a => a.id === id);
      if (!item) return prev;
  ...
  const getItemLabel = useCallback((id: string, type: 'part' | 'assembly') => {
    if (type === 'part') {
      ...
      return p ? `[${p.code}] ${p.name} (¥${p.price.toFixed(2)})` : '未知零件';
    }
                ...
                <div className="divide-y divide-slate-50">
                  {currentList.length === 0 ? (
                    ...
                    currentList.map(item => {
                      ...
                      const label = activeTab === 'part'
                        ? `[${(item as Part).code}] ${(item as Part).name} - ${(item as Part).spec}`
                        : `[${(item as Assembly).code}] ${(item as Assembly).name}`;
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <span className="text-xs font-mono text-slate-500">
                              {activeTab === 'part' ? (item as Part).code : (item as Assembly).code}
                            </span>
                            <span className="text-sm ml-1.5 text-slate-700">{label.split('] ')[1] || label}</span>
                          </div>
